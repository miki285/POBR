...
{
	cv::Mat flooded_image = copyImageBinaryToRGB(image);
	...
	while (!pointQueue.empty()) {
		...
		flooded_image.at<cv::Vec3b>(tempX, tempY)[0] = color[0];
		flooded_image.at<cv::Vec3b>(tempX, tempY)[1] = color[1];
		flooded_image.at<cv::Vec3b>(tempX, tempY)[2] = color[2];
		cv::imshow("test", flooded_image);
		cv::waitKey(1);
		...
		if (left.x >= 0 && flooded_image.at<cv::Vec3b>(left.x, left.y)[0] == 255 && std::find(pointQueue.begin(), pointQueue.end(), left) == pointQueue.end()) {
			pointQueue.push_back(left);
		}
		if (right.x < flooded_image.cols && flooded_image.at<cv::Vec3b>(right.x, right.y)[0] == 255 && std::find(pointQueue.begin(), pointQueue.end(), right) == pointQueue.end()) {
			pointQueue.push_back(right);
		}

		if (up.y >= 0 && flooded_image.at<cv::Vec3b>(up.x, up.y)[0] == 255 && std::find(pointQueue.begin(), pointQueue.end(), up) == pointQueue.end()) {
			pointQueue.push_back(up);
		}
		if (down.y < flooded_image.rows && flooded_image.at<cv::Vec3b>(down.x, down.y)[0] == 255 && std::find(pointQueue.begin(), pointQueue.end(), down) == pointQueue.end()) {
